...
import { ArrowRightInbox } from "@upyog/digit-ui-react-components";
import { Link } from "react-router-dom";

const ReportModuleCard = ({ Icon, moduleName, kpis = [], links = [], isCitizen = false, className, styles, FsmHideCount }) => {
  return (
            ...
            <div className="links-wrapper" style={{ width: "100%", display: "flex", fontSize: "0.8rem", paddingLeft: "10px", flexWrap:"wrap",flexDirection:"row",paddingTop:"10px"}}>
              {links.map(({ count, label, link }, index) => (
                <div className="link" key={index} style={{ paddingLeft: "5px", color: "#a1a5b7",display:"flex" }}>
                  {link ? <div style={{display:"flex"}}> <a href={link}>{label}</a>  <span>|</span> </div>: null}
                </div>
...
const ReportCardFullWidth = ({ moduleName,  links = [], isCitizen = false, className, styles, headerStyle, subHeader, subHeaderLink }) => {
  return (
...
export { ReportModuleCard, ReportCardFullWidth };